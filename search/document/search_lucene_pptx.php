 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class search_lucene_pptx extends Zend_Search_Lucene_Document_OpenXml
{
    /**
     ...
     * Object constructor
     *
     * @param search_document $doc
     * @param boolean $storeContent
     * @throws Zend_Search_Lucene_Exception
     */
    public function __construct(search_document $doc, $storeContent=false)
    {
        ...
        foreach ($relations->Relationship as $rel) {
            if ($rel["Type"] == Zend_Search_Lucene_Document_OpenXml::SCHEMA_OFFICEDOCUMENT) {
                ...
                foreach ($slideRelations->Relationship as $slideRel) {
                    if ($slideRel["Type"] == search_lucene_pptx::SCHEMA_SLIDERELATION) {
                        // Found slide!
                        ...
                        foreach ($slideNotesRelations->Relationship as $slideNoteRel) {
                            if ($slideNoteRel["Type"] == search_lucene_pptx::SCHEMA_SLIDENOTESRELATION) {
                                // Found slide notes!
        ...
        foreach ($slides as $slideKey => $slide) {
            // Register namespaces
            $slide->registerXPathNamespace("p", search_lucene_pptx::SCHEMA_PRESENTATIONML);
            $slide->registerXPathNamespace("a", search_lucene_pptx::SCHEMA_DRAWINGML);
            ...
            if (isset($slideNotes[$slideKey])) {
                ...
                // Register namespaces
                $slideNote->registerXPathNamespace("p", search_lucene_pptx::SCHEMA_PRESENTATIONML);
                $slideNote->registerXPathNamespace("a", search_lucene_pptx::SCHEMA_DRAWINGML);
        ...
        //$this->addField(Zend_Search_Lucene_Field::Text('filename', $fileName, 'UTF-8'));
            ...
            // Store contents
        if ($storeContent) {
            $this->addField(Zend_Search_Lucene_Field::Text('content', implode(' ', $documentBody), 'UTF-8'));
        } else {
            $this->addField(Zend_Search_Lucene_Field::UnStored('content', implode(' ', $documentBody), 'UTF-8'));
        }
/*
