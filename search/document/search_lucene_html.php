 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class search_lucene_html extends Zend_Search_Lucene_Document
{
    /**
     ...
     * Object constructor
     *
     * @param search_document $doc         HTML string (may be HTML fragment, )
     * @param boolean $storeContent
     * @param string  $defaultEncoding   HTML encoding, is used if it's not specified using Content-type HTTP-EQUIV meta tag.
     */
    public function __construct(search_document $doc, $storeContent=false, $defaultEncoding = '')
    {
        $this->doc = $doc;
        ...
        if ($doc->get_content()) {
          $htmlData = $doc->get_content();  
        } else {
            $htmlData = file_get_contents($doc->get_filepath());
        }
...
*/
        ...
        if ($storeContent) {
            $this->addField(Zend_Search_Lucene_Field::Text('content', $docBody, 'UTF-8'));
        } else {
            $this->addField(Zend_Search_Lucene_Field::UnStored('content', $docBody, 'UTF-8'));
        }
